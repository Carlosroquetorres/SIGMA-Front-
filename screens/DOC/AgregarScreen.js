...
const AgregarScreen = ({ navigation  }) => {
  ...
  const closeModalAndNavigate = () => {
    ...
    navigation.navigate('AgregarDoc'); // Cambia si es necesario
  };

  const handleFormSubmit2 = () => {

    if (!name.trim() || !code.trim() || !user.trim() || !Descript.trim() ) {
      setError('Error , Por favor, complete todos los campos del archivo. (nombre, codigo , usuario quien sube y la descripcion del archivo)');
      return;
    }
  ...
  return (
    <ScrollView scrollEnabled={false} style={styles.back}>
      ...
      <Text style={styles.text22}>SUBIR DOCUMENTO</Text>
      </View>  
     ...
     <TouchableOpacity onPress={() => navigation.navigate('DocHome')}>
      ...
      <Text style={styles.b3}>AGREGAR DOCUMENTO</Text>
      </TouchableOpacity>
     </Animatable.View>
     ...
     <TouchableOpacity onPress={() => navigation.navigate('BuscDoc')} style={styles.button}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('EditHomeDoc')} style={styles.button}>
        ...
        <TextInput
                style={styles.input}
                placeholder="Ingrese nombre de quien sube"
                value={user}
