...
import Footer from './comp/FooterScreen';
  ...
  const AdminRevScreen = ({ navigation  }) => {
    ...
    const [datos, setDatos] = useState([]);
      ...
      <View style={styles.cell}><Text style={styles.tx}>{item.status}</Text></View>
        <View style={styles.cell2}> <Button style={styles.button} title="Actualizar" onPress={() => abrirModal1(item.folio,item.department,item.status)} color="#1F3D6D"/></View>
  ...
  return (
    ...
    <View style={styles.table}>
      ...
      <View style={styles.row1}>
        ...
        <Text style={styles.cell1}>ENVIAR</Text>
