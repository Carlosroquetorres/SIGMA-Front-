...
const FormAScreen = () => {
  ...
  const handleFormSubmit2 = () => {
    ...
    if (!mail.trim() || !nameD.trim() || !lastnameD.trim() || !gendre2.trim() || !puesto2.trim() ||
        !entityD.trim() || !dateH.trim() || !hour.trim() || !place.trim() || !facts.trim() || !descHechos.trim()) {
      setError('Error , Por favor, complete todos los campos. (Su corre electrónico, en el caso del denunciado (nombre, apellido, genero, cargo, entidad,fecha de lo ocurrido, hora, frecunecia, lugar y la descripcion de los hechos))');
      return;
    }
